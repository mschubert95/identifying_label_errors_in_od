...
class BBoxHead(BaseModule):
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            # print(labels)
            # print(len([x for x in labels if x == 999]))
            # labels = [x for x in labels if x != 999]
            if not bg_regression:
                bg_class_ind = self.num_classes
                # 0~self.num_classes-1 are FG, self.num_classes is BG
                pos_inds = (labels >= 0) & (labels < bg_class_ind)
            else:
                pos_inds = (labels >= 0) & (labels < 999)
    ...
    def get_bboxes_heatmap(self,
        ...
        #     return bboxes.cpu().unsqueeze(0), scores.cpu().unsqueeze(0), None
            ...
            # print("rois:", rois)
            det_bboxes, det_labels, det_inds, mlv_scores = multiclass_nms(bboxes, scores,
                                                    cfg.score_thr, cfg.nms,
                                                    cfg.max_per_img,
                                                    return_inds=True,
                                                    return_scores=True,
                                                    loss_as_score=loss_as_score)
                                                        
            return det_bboxes, det_labels, det_inds, mlv_scores
